  ...
  return (
    <div className="page-enter max-w-6xl mx-auto px-6 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm mt-1">System-wide monitoring overview</p>
      </div>
      ...
      <div className="glass-card p-8 mb-8">
        <div className="flex items-center justify-between flex-wrap gap-6">
          <div>
            <p className="text-sm text-gray-500 mb-2">Overall System Health</p>
            <div className="flex items-end gap-3">
              <span className="text-5xl font-black font-mono" style={{
                background: 'linear-gradient(135deg, #10b981, #06b6d4)',
                WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent'
              }}>{overallUptime}%</span>
              <span className="text-gray-600 text-sm mb-2">uptime</span>
            </div>
          </div>
          <div className="flex gap-6">
            <div className="text-center">
              <p className="text-3xl font-bold" style={{ color: '#10b981' }}>{upCount}</p>
              <p className="text-xs text-gray-500 mt-1">Online</p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-bold" style={{ color: '#ef4444' }}>{downCount}</p>
              <p className="text-xs text-gray-500 mt-1">Down</p>
            </div>
            <div className="text-center">
              <p className="text-3xl font-bold" style={{ color: '#7c3aed' }}>{monitors.length}</p>
      ...
      {monitors.length > 0 ? (
        <div className="glass-card overflow-hidden">
          <div className="p-5 border-b border-white/[0.04]">
            <h3 className="font-bold">All Monitors</h3>
          </div>
          <div className="divide-y divide-white/[0.03]">
            {monitors.map(m => (
              <Link key={m._id} to={`/status/${m._id}`} className="flex items-center gap-4 p-5 hover:bg-white/[0.02] transition-colors no-underline">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-600 font-mono truncate">{m.url}</p>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-sm font-mono" style={{ color: '#06b6d4' }}>
                    {uptimes[m._id] ? `${uptimes[m._id]}%` : '—'}
                  </p>
                  <p className="text-xs text-gray-600">24h uptime</p>
                </div>
        ...
        <div className="glass-card p-12 text-center">
          <p className="text-gray-500">No monitors to analyze. Add monitors from the Dashboard.</p>
